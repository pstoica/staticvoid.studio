...
| `.silhouette(mode?, snap?)` | mode: `1` inside · `-1` outside · `0` off. snap: when to (re)capture | mask the group by the **person in the webcam** (segmentation, soft edges): `1` = the group exists only inside your body, `-1` = you're a hole in it. Patternable (`"<1 -1>"`). **`snap`** freezes poses: truthy on a frame recaptures the mask, then it HOLDS — default `1` follows you live; `"<1 0 0 0>"` stamps your pose once per 4 cycles; `gate(1)` stamps on a MIDI hit; `mouseDown` on click. Layer groups with offset snap patterns for a trail of frozen body-echoes. Lazy — using it loads the model + camera. Chain order is the fun: `.silhouette().feedback(…)` lets trails escape your body; `.feedback().silhouette()` clips the fog to it |
...
| `shaken(id)` | a deliberate **shake**, as a decaying pulse; amplitude scales with shake intensity | `0..1` |
| `held(id)` | `1` while the ball is **held still** (latched by hold/active gestures, not decayed) — a mode you can hold a visual on | `0` / `1` |
